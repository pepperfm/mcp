...
/**
 * Repo Docs MCP (Bun) — generic MCP stdio server.
 *
...
const SERVER_NAME = (process.env.MCP_SERVER_NAME ?? `${TOOL_PREFIX}-docs`).trim();
const SERVER_VERSION = (process.env.MCP_SERVER_VERSION ?? "0.2.0").trim();

const AUTO_GIT_FETCH = String(process.env.MCP_GIT_AUTO_FETCH ?? "").trim() === "1";
...
function runGitFetch(): Promise<{ ok: boolean; code: number | null; stdout: string; stderr: string }> {
...
async function main() {
  ...
  console.error(`${SERVER_NAME} MCP server running (stdio)`);
